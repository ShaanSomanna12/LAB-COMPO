...
const defaultInventory = [
  { id: 1, name: 'Raspberry Pi 4 Model B', available: 4, total: 5, desc: 'High-performance single-board computer.', department: 'CSE', location: 'Lab 201', status: 'Available' },
  ...
  { id: 3, name: 'NVIDIA Jetson Nano', available: 2, total: 2, desc: 'Small, powerful computer for embedded AI.', department: 'CSE', location: 'AI Lab 404', status: 'Available' },
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { name, department, total, desc, location } = body;
    const res = await query(
      'INSERT INTO components (name, department, total_quantity, available_quantity, base_condition, lab_location) VALUES ($1, $2, $3, $3, $4, $5) RETURNING *',
      [name, department, total, desc || 'New catalog item', location || 'Main Lab']
    );
    ...
    const newItem = { id: Date.now(), name: body.name, department: body.department, total: body.total, available: body.total, desc: body.desc, location: body.location, status: 'Available' };
